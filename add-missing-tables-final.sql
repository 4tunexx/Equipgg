-- =============================================
-- ADD MISSING TABLES FOR EQUIPGG PLATFORM (FINAL FIX)
-- =============================================
...
-- =============================================
-- ADD ROW LEVEL SECURITY (RLS) POLICIES - FIXED TYPE CASTING
-- =============================================

-- Support tickets RLS
ALTER TABLE public.support_tickets ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view own tickets" ON public.support_tickets FOR SELECT USING (user_id::TEXT = auth.uid()::TEXT);
CREATE POLICY "Users can create tickets" ON public.support_tickets FOR INSERT WITH CHECK (user_id::TEXT = auth.uid()::TEXT);
CREATE POLICY "Admins can view all tickets" ON public.support_tickets FOR SELECT USING (EXISTS (SELECT 1 FROM public.users WHERE id::TEXT = auth.uid()::TEXT AND role = 'admin'));

-- Trade offers RLS
ALTER TABLE public.trade_offers ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view own trade offers" ON public.trade_offers FOR SELECT USING (sender_id::TEXT = auth.uid()::TEXT OR receiver_id::TEXT = auth.uid()::TEXT);
CREATE POLICY "Users can create trade offers" ON public.trade_offers FOR INSERT WITH CHECK (sender_id::TEXT = auth.uid()::TEXT);
CREATE POLICY "Users can update own trade offers" ON public.trade_offers FOR UPDATE USING (sender_id::TEXT = auth.uid()::TEXT OR receiver_id::TEXT = auth.uid()::TEXT);

-- Polls RLS
ALTER TABLE public.polls ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Anyone can view active polls" ON public.polls FOR SELECT USING (is_active = true);
CREATE POLICY "Admins can manage polls" ON public.polls FOR ALL USING (EXISTS (SELECT 1 FROM public.users WHERE id::TEXT = auth.uid()::TEXT AND role = 'admin'));

-- Shop items RLS
ALTER TABLE public.shop_items ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Anyone can view shop items" ON public.shop_items FOR SELECT USING (is_active = true);
CREATE POLICY "Admins can manage shop items" ON public.shop_items FOR ALL USING (EXISTS (SELECT 1 FROM public.users WHERE id::TEXT = auth.uid()::TEXT AND role = 'admin'));

-- User perk claims RLS
ALTER TABLE public.user_perk_claims ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view own perk claims" ON public.user_perk_claims FOR SELECT USING (user_id::TEXT = auth.uid()::TEXT);
CREATE POLICY "Users can create perk claims" ON public.user_perk_claims FOR INSERT WITH CHECK (user_id::TEXT = auth.uid()::TEXT);

-- Inventory items RLS
ALTER TABLE public.inventory_items ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view own inventory items" ON public.inventory_items FOR SELECT USING (user_id::TEXT = auth.uid()::TEXT);
CREATE POLICY "Users can manage own inventory" ON public.inventory_items FOR ALL USING (user_id::TEXT = auth.uid()::TEXT);

-- Steam trade offers RLS
ALTER TABLE public.steam_trade_offers ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view own steam trade offers" ON public.steam_trade_offers FOR SELECT USING (user_id::TEXT = auth.uid()::TEXT);
CREATE POLICY "Users can create steam trade offers" ON public.steam_trade_offers FOR INSERT WITH CHECK (user_id::TEXT = auth.uid()::TEXT);

-- Transactions RLS
ALTER TABLE public.transactions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can view own transactions" ON public.transactions FOR SELECT USING (user_id::TEXT = auth.uid()::TEXT);
