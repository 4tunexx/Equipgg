-- EQUIPGG INSTANT PRODUCTION READINESS - FIXED FOR YOUR ACTUAL DATABASE SCHEMA
-- This script only uses tables that exist in your Supabase database

-- ==============================================
-- 1. ADD MISSING CRITICAL COLUMNS (SAFE)
-- ==============================================
...
-- ==============================================
-- 2. POPULATE USER DATA (ONLY EXISTING TABLES)
-- ==============================================
...
BEGIN
    ...
    IF user_count > 0 THEN
        ...
        IF user_achievements_count = 0 THEN
            ...
            -- First check what columns exist in user_achievements table
        ...
        ELSE
            RAISE NOTICE 'User achievements already exist (% records)', user_achievements_count;
        END IF;
        ...
        ELSE
            RAISE NOTICE 'User inventory already exists (% records)', user_inventory_count;
        END IF;
        ...
        ELSE
            RAISE NOTICE 'Notifications already exist (% records)', notifications_count;
        END IF;
...
-- ==============================================
-- 4. CREATE FLASH SALES FOR SHOP (IF TABLE EXISTS)
-- ==============================================

-- Create flash sales if table exists and none exist
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'flash_sales') THEN
        ...
        RAISE NOTICE 'Created flash sales';
    ELSE
        RAISE NOTICE 'Flash sales table does not exist - skipping';
    END IF;
END $$;
...
BEGIN
    ...
    -- Check flash sales if table exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'flash_sales') THEN
